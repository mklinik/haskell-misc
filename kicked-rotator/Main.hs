...
-- one random point (x, y) in the interval [-pi * 2, pi * 2]
...
-- insert iterations number of pixels of the given color in a map
...
main = do
  ...
  let width = 600
  let height = 600
  startPoints <- replicateM 80 randomPoint
  ...
  putStr $ map2ppm pixelMap width height
